...
test.describe('Checkout critical flows', () => {
  ...
  test('should add item to cart', { tag: '@smoke' }, async ({ productPage, cartPage}) => {
     ...
     await test.step('Add first product to cart', async () => {
        ...
        selectedPrice = await firstProduct.getPrice();
        await test.step('Add first product to cart', async () => {
  ...
  test('should complete checkout successfully', { tag: '@smoke' }, async ({page, productPage, cartPage, checkoutPage, confirmationPage}) => {
    ...
    await test.step('Fill shipping form and place order', async () => {
      ...
      expect(await confirmationPage.confirmationMessage.textContent()).toContain('Your Order has been successfully placed');
